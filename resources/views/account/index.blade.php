        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-10 flex flex-col gap-y-5">
                <div class="flex flex-col md:flex-row justify-between items-center mb-6">
                    <a href="{{ route('account.create') }}" class="bg-indigo-600 hover:bg-indigo-300 text-white font-bold py-2 px-4 rounded-lg transition duration-300 flex items-center justify-center">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" viewBox="0 0 20 20" fill="currentColor">
                            <path fill-rule="evenodd" d="M10 3a1 1 0 011 1v5h5a1 1 0 110 2h-5v5a1 1 0 11-2 0v-5H4a1 1 0 110-2h5V4a1 1 0 011-1z" clip-rule="evenodd" />
                        </svg>
                        Tambah Akun
                    </a>
                    <form method="GET" action="{{ route('account.index') }}" class="flex w-full md:w-auto">
                        <input type="text" name="search" placeholder="Cari data akun"
                    ...
                    <tbody id="users-table-body">
                        @forelse ($users as $user)
                            <tr class="hover:bg-indigo-50">
                                <td class="border-b border-gray-200 px-6 py-4">{{ $user->identity_no }}</td>
                                <td class="border-b border-gray-200 px-6 py-4">{{ $user->name }}</td>
                                <td class="border-b border-gray-200 px-6 py-4">{{ $user->position }}</td>
                                <td class="border-b border-gray-200 px-6 py-4">
                                    <a href="{{ route('account.show', $user->id) }}" class="font-bold py-2 px-4 bg-indigo-700 hover:bg-indigo-400 text-white rounded-lg">View</a>
                                </td>
                            <tr>
                        @empty
                        <tr>
